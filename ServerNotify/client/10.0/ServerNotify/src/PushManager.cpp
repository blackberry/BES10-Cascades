...
PushManager::PushManager(Application *app) :
	...
	debugLabel = app->findChild<Label*>("debugLabel");
	log("Logging enabled if this shows up.");
...
void PushManager::invoked(const InvokeRequest& request) {
	...
	if (payload.isValid()) {
		...
		if (payload.isAckRequired()) { //This section is only useful for the Notify URL
		...
		case PushManager::Medium:
			handleMediumPriorityPush(messageParts.at(1), messageParts.at(2), messageParts.at(3));
			break;
		case PushManager::High:
			handleHighPriorityPush(messageParts.at(1), messageParts.at(2), messageParts.at(3));
			break;
		default:
			break;
		}
...
void PushManager::debug(const QString &str){
	QString temp = debugLabel->text() + "\n" + str;
	//uncomment to enable debugging
	//debugLabel->setText(temp);
