...
SimplePushClient::SimplePushClient(bb::cascades::Application *app) :
	...
	//Get a handle to the logging label in QML
	m_logOutputLabel = root->findChild<Label*>("logger");
	...
	m_pushGroupDataModel = new GroupDataModel(
			QStringList() << "priority" << "title" << "body" << "url" << "timeReceived");
...
void SimplePushClient::pushListUpdated() {
	...
	foreach(QVariant pushMessageVariant, pushList){
		...
		if (messageParts.size() != 5){
			qDebug() << "Invalid list length. Expected 5, received: " << messageParts.size() << "\nSkipping this item";
			continue;
		}
		...
		pushMessageMap["url"] = messageParts[3];
		pushMessageMap["timeReceived"] = messageParts[4];
		m_pushGroupDataModel->insert(pushMessageMap);
		qDebug() << "Data insertion successful!";
	}

	m_logOutputLabel->setText(settings.value("logger", "empty").toString());
}
...
void SimplePushClient::clearPushList() {
	...
	request.setTarget("com.abccompany.servernotifyservice");
	request.setAction("com.abccompany.servernotify.CLEAR_HISTORY");
	request.setMimeType("application/com.abccompany.servernotify.CLEAR_HISTORY");
