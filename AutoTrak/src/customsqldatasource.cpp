...
bool CustomSqlDataSource::checkConnection()
{
    if (mSqlConnector) {
        ...
        if (newFile.exists()) {
            ...
            mSqlConnector = new SqlConnection(mSourceInDataFolder, "connect", this);
...
void CustomSqlDataSource::executeBatch(const QString& query,
		const QVariantList& valuesByName, int id) {

    if (checkConnection()) {
        mSqlConnector->executeBatch(query, valuesByName, id);
    }
}

void CustomSqlDataSource::beginTransaction() {
    if (checkConnection()) {
        mSqlConnector->beginTransaction(-1);
    }
}

void CustomSqlDataSource::endTransaction() {
    if (checkConnection()) {
    	mSqlConnector->endTransaction(-1);
    }
}
