...
Container {
    ...
    Container {
        ...
        //each of the carousel items are set up here (properties, background images applied, etc)
        CarouselItem {
            id: bbmInvite
            objectName: "bbmInvite"
            centered: true
            angle: root.angle + root.baseAngle
            imgFileName: "images/bbmInvite.png"
        ...
        CarouselItem {
            id: bbmShareText
            objectName: "bbmShareText"
            angle: root.angle + root.baseAngle
            imgFileName: "images/bbmShareText.png"
        }
        CarouselItem {
            id: bbmShareTextGrp
            objectName: "bbmShareTextGrp"
            angle: root.angle + root.baseAngle
            imgFileName: "images/bbmShareTextGrp.png"
        }
        CarouselItem {
            id: bbmShareImage
            objectName: "bbmShareImage"
            angle: root.angle + root.baseAngle
            imgFileName: "images/bbmShareImage.png"
        }
        CarouselItem {
            id: bbmShareImageGroup
            objectName: "bbmShareImageGrp"
            angle: root.angle + root.baseAngle
            imgFileName: "images/bbmShareImageGrp.png"
        }
        //        CarouselItem {
//            id: my5
//            angle: root.angle + root.baseAngle
...
//        }    
    ...
    onCreationCompleted: {
        ...
        for (var i = 0; i < carouselContainer.controls.length; i ++) {
         ...
         //   myCarouselToast.body = carouselContainer.controls[i].objectName;
         //   myCarouselToast.show();

            navigationPane.imageCenter = carouselContainer.controls[i].objectName;

        }
        root.centeredControl = carouselContainer.controls[0]

        //set the main variant for the image that is centered 
        //navigationPane.imageCenter = carouselContainer.controls[0].objectName;
