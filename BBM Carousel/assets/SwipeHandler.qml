...
Container {
    ...
    onTouch: {
        if (event.isDown()) {
            ...
            if (Math.abs(dragOffset) > 125 && Math.abs(speed) > 1) {
                ...
                if(navigationPane.imageCenter == "bbmInvite"){

                    app.invokeBBMInvite();
                
                } else if (navigationPane.imageCenter == "bbmShareText") {

                    app.invokeBBMShareText();

                } else if (navigationPane.imageCenter == "bbmShareTextGrp"){

                    app.invokeBBMShareTextGrp();

                } else if (navigationPane.imageCenter == "bbmShareImage"){

                    app.invokeBBMShareImage();

                } else if (navigationPane.imageCenter == "bbmShareImageGrp") {

                    app.invokeBBMShareImageGrp();
    ...
    attachedObjects: [
        ...
        AlertDialog {
            ...
            cancelButton: true
            okButton: true
            alertMessage: "If you're seeing this error, RUN! \r\r "
            titleText: "WARNING!!!!"
            onOk: {
               // navigationPane.pop();
            }
